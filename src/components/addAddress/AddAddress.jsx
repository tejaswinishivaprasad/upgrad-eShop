...
import { addAddressToDb, getAddresses, getProductForGivenID, createOrder, userData } from '../../common/services/apiService';
...
const AddAddress = () => {
    ...
    const addAddress = async () => {
        ...
        const isAllRequiredFieldsFilled = requiredFields.every(field => data[field].trim().length > 0);
    ...
    const placeOrder = async () => {
        const addressValues = Object.values(address).filter(value => typeof value === 'string' && value.trim() !== '');
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <Box sx={{ width: '80%', height: '100%', margin: 'auto', marginY: '20px' }}>
...
>
    ...
    {addressList.map((e, i) => (
        <MenuItem key={i} value={e.id}> {/* Use address ID as value */}
            {`${e.name}, ${e.street}, ${e.city}, ${e.state}, ${e.zipcode}`} {/* Display address details */}
        </MenuItem>
    ))}
</TextField>
                    ...
                    <form className='registerForm1' style={{ marginTop: '20px' }}>
                        ...
                        <TextField name='contactNumber' variant='outlined' label='Contact Number *' onChange={handleChange} />
                        <TextField name='street' variant='outlined' label='Street *' onChange={handleChange} />
                        <TextField name='city' variant='outlined' label='City *' onChange={handleChange} />
                        <TextField name='state' variant='outlined' label='State *' onChange={handleChange} />
                        <TextField name='landmark' variant='outlined' label='Landmark' onChange={handleChange} />
                        <TextField name='zipcode' variant='outlined' label='Zipcode *' onChange={handleChange} />
                        <button type='button' onClick={addAddress} className='loginBtn'>Save Address</button>
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <Button style={{ marginTop: '10px', marginRight: '20px', color: 'black' }} size="medium" variant='text' onClick={() => navigate('/product/' + id)}>Back</Button>
                            <Button style={{ marginTop: '10px', backgroundColor: "#3f51b5" }} size="medium" variant='contained' color="primary" onClick={handleNext}>Next</Button>
                        </div>
                    </form>
                </div>
            </>}
            {activeKey === 2 && <>
                <div className='finalStep'>
                    ...
                    <div style={{ width: '40%', borderLeft: '2px solid #80808045' }}>
                        ...
                        <p>Contact Number: {addressList.find(a => a.id === address)?.contactNumber}</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', marginTop: '25px' }}>
                    <Button style={{ marginTop: '10px', marginRight: '20px', color: 'black' }} size="medium" variant='text' onClick={handleBack}>BACK</Button>
